...
var mythemeAdmin = {
      ...
      combo.each(function(){
        jQuery(this).unbind('change').bind('change',function(){
          ...
          if(jQuery(this).val()){
            ...
            jQuery(this).parents("div.multidropdown").remove();
            jQuery(this).remove();
          }
           mythemeAdmin.multiDropdown();
        }); //change end
    ...
    jQuery("a.mytheme-import-button").bind('click',function(e){
      ...
      if(jQuery(this).hasClass('import-disabled')) {
        ...
        if(confirm(objectL10n.importConfirm)){
          ...
          var $dummy =  'dream-spa-dummy'; // replace this line with bellow line and enable the line no : 336
		...
		$ptemplate_select.live('change', function(){
			...
			switch($val){
				case 'tpl-blog.php':
					...
					$ptemplate_box.find("#page-sidebars").slideDown();
				...
				case 'tpl-fullwidth.php':
					...
					$ptemplate_box.find("#page-sidebars").slideUp();
				break;

				case 'tpl-reservation.php':
					$ptemplate_box.find('span:first').text('Reservation page Options');
					$ptemplate_box.slideDown();
					$ptemplate_box.find('#tpl-common-settings').slideDown();
					$ptemplate_box.find('#tpl-default-settings').slideDown();
					$ptemplate_box.find('#custom-style-settings').slideDown();
  ...
  addPrice : function() {
    jQuery(".add-price-item").click(function(event) {
      $parent = jQuery(this).parents(".property-price");
      $clone = jQuery(this).siblings(".clone").find(".property-price-container").clone();
      console.log( $clone );
  ...
  removePrice : function(){
    jQuery("body").delegate(".remove-price-item","click",function(event){
      jQuery(this).parents(".property-price-container").remove();
      event.preventDefault();
    });
  }, //Remove Price
